...
// Claude is natively multilingual, so language support is just a directive.
// English is the default (no directive → unchanged behavior).
const LANGUAGE_NAMES = { en: 'English', es: 'Spanish' }

function languageDirective(lang) {
  const name = LANGUAGE_NAMES[lang]
  if (!name || lang === 'en') return ''
  return `\n\nIMPORTANT: Write your entire response — the theme label, the reflection, and the affirmation — in ${name}. Use warm, natural, native ${name}, not a word-for-word translation of English.`
}
...
export async function composeAffirmation({ entry, lastAffirmation, lang } = {}) {
